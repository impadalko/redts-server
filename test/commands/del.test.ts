...
import delProcessor from "@src/commands/del.ts";
...
describe("Get Processor", () => {
  ...
  it("should return an error when input length validator returns an error", () => {
    ...
    const output = delProcessor.process(command, args);
  ...
  it("should return the amount of deleted keys when they are present", () => {
    mockInputLengthValidator(undefined);
    mockNotNullValidator(undefined);
    storageStub = stub(storage, "del", () => true);

    let output = delProcessor.process(command, args);

    assertEquals(output.type, "integer");
    assertEquals(output.payload, 1);

    output = delProcessor.process(command, multipleArgs);
  ...
  it("should return zero when no keys are present", () => {
    mockInputLengthValidator(undefined);
    mockNotNullValidator(undefined);
    storageStub = stub(storage, "del", () => false);
