...
import getProcessor from "src/commands/get.ts";
...
describe("Get Processor", () => {
  ...
  it("should return an error when input length validator returns an error", () => {
    ...
    const output = getProcessor.process(command, args);
  ...
  it("should return nil when storage returns undefined", () => {
    mockInputLengthValidator(undefined);
    mockNotNullValidator(undefined);
    storageStub = stub(storage, "get", () => undefined);

    const output = getProcessor.process(command, args);

    assertEquals(output.type, "nil");
  });

  it("should return a string when storage returns a string", () => {
    mockInputLengthValidator(undefined);
    mockNotNullValidator(undefined);
    storageStub = stub(storage, "get", () => storedValue);

    const output = getProcessor.process(command, args);

    assertEquals(output.type, "simpleString");
    assertStringIncludes(output.encode(), storedValue);
  });
